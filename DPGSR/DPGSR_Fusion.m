function [pred_label, acc, out] = DPGSR_Fusion(S, T, S_label, T_label, nt, lambda1, lambda2, mode, fixedAlpha, solver, verif_thresh, pr_margin_thresh, lambda3)
% DPGSR_Fusion
%
...
if nargin < 13 || isempty(lambda3)
    lambda3 = 0;  % 0 = 标准逆向, >0 = 稀疏逆向
end
...
if strcmpi(solver, 'joint')
    ...
    W = Forward(S, T, nt, lambda1);     % W size: N x M
    ...
    if lambda3 > 0
        D = ReverseSparse(T, S, nt, lambda2, lambda3);
    else
